...
export class ResultDisplay {
  ...
  setupElements() {
    ...
    this.resultTabs =
      document.getElementById('result-tabs');
    this.resultContent = document.getElementById(
      'result-content'
    );
  ...
  displayResults(results) {
    ...
    console.log('Results.txt:', results.txt);
    console.log('Results.textBlocks:', results.textBlocks);
  ...
  displayTextBlocks(textBlocks) {
    const blocksElement =
      document.getElementById('text-blocks');
      ...
      .map((block, index) => {
        ...
        <div class="text-block" data-block="${index}">
          <div class="block-header">
            ...
            <span class="block-position">位置: (${
              block.x
            }, ${block.y})</span>
            <span class="block-size">サイズ: ${
              block.width
            }×${block.height}</span>
            <span class="block-confidence">信頼度: ${confidence}%</span>
          </div>
          <div class="block-text">${this.escapeHtml(
            text
          )}</div>
        </div>
      `;
  ...
  displayTextResult(results) {
    ...
    textElement.innerHTML = `
      <pre class="result-content-text">${this.escapeHtml(
        textContent
      )}</pre>
    `;
  }
  ...
  displayXMLResult(xmlData) {
    const xmlElement =
      document.getElementById('result-xml');
  ...
  switchTab(tabId) {
    ...
    // アクティブなタブを更新
    const tabButtons =
      this.resultTabs.querySelectorAll('.tab-button');
    ...
    const contentSections =
      this.resultContent.querySelectorAll('.tab-content');
  ...
  generateXMLContent() {
    ...
    this.currentResults.textBlocks.forEach(
      (block, index) => {
  ...
  enableDownloadButtons() {
    ...
    downloadButtons.forEach((button) => {
      ...
      switch (format) {
        ...
        case 'json':
          enabled = !!(
            this.currentResults?.json ||
            this.currentResults?.textBlocks
          );
          break;
        case 'xml':
          enabled = !!(
